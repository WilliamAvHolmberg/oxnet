class HiscoresController < ApplicationController
  ...
  # GET /hiscores/1.json
  def show
    @accounts = Account.all.select{|acc| acc.stats != nil && acc.stats.where(skill: @hiscore.skill).first != nil }.sort_by{|acc| acc.stats.where(skill: @hiscore.skill).first.level}.reverse
  end
