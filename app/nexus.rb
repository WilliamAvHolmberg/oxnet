...
def get_mule_withdraw_task_respond(account)
  mule_withdraw_tasks = MuleWithdrawTask.where("created_at >= ?", Time.zone.now.beginning_of_day, :account => account).select{|task| !task.executed && task.is_relevant  && !task.account!= nil && task.account.id == account.id }
